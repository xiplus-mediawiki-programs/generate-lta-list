...
class GererateLtaList:
    def __init__(self, args):
        ...
        self.logger = logging.getLogger('generate_lta_list')
    ...
    def main(self):
        pages = set()
        for page in pywikibot.pagegenerators.PrefixingPageGenerator('LTA:', includeredirects=True):
            pages.add(page.title())
        pages = sorted(pages)

        page_list = '''<!--
本列表由程式自動產生，任何手動修改將在下次更新時被覆蓋
程式碼：https://github.com/xiplus-mediawiki-programs/generate-lta-list
-->
'''
        for page in pages:
            page_list += '# [[{}]]\n'.format(page)

        page = pywikibot.Page(self.site, 'Wikipedia:最近更改巡查/頁面列表/LTA重新導向')
        old_text = page.text
...
if __name__ == '__main__':
    ...
    gll = GererateLtaList(args)
    gll.logger.setLevel(args.loglevel)
    gll.logger.debug('args: %s', args)
    gll.main()
